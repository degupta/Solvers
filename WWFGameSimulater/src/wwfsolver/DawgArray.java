...
public class DawgArray {
	/** Singleton instance */
	private static final DawgArray sSingleton = new DawgArray();

	/** Singleton accessor
	 * 
	 * @return singleton instance */
	public static DawgArray getInstance() {
		return DawgArray.sSingleton;
	}
	...
	public DawgArray(final InputStream in) {
		this.createDawgFromFileForArrayLookup(in);
	}
	...
	public void createDawgFromFileForArrayLookup(final InputStream in) {
		try {
			int val = 0, i = 0;
			// The first four bytes will specify how many bytes are there in the
			// file (minus the first four bytes)
			final int size = (in.read() << 24) | (in.read() << 16) | (in.read() << 8) | in.read();
			// Create an array of that size
			this.dawgArray = new byte[size];
			// Read in array byte by byte
			while ((val = in.read()) != -1) {
				this.dawgArray[i++] = (byte) (val);
			}
	...
	public boolean wordExists(final MyStringBuilder word) {
		final int length = word.position;
		if (length == 0) {
			return false;
		}
		...
		while (true) {
			...
			childPos = (this.unsignedToBytes(this.dawgArray[pos]) << 16) | (this.unsignedToBytes(this.dawgArray[pos + 1]) << 8) | (this.unsignedToBytes(this.dawgArray[pos + 2]));
			childPos = childPos >> 2;
			...
			while (true) {
				...
				currentChar = (char) (this.dawgArray[childArrPos + 3]);
				...
				if ((this.dawgArray[childArrPos + 2] & 2) > 0) {
			...
			if (currentIndex == length) {
				return (this.dawgArray[childArrPos + 2] & 1) > 0; // Is Final
																	// Node
			}
	 ...
	 * end-of-word */
	public boolean isChildEndOFWord(final int currentNode, final char letter) {
		final int pos = currentNode * 4;
		int childPos = (this.unsignedToBytes(this.dawgArray[pos]) << 16) | (this.unsignedToBytes(this.dawgArray[pos + 1]) << 8) | (this.unsignedToBytes(this.dawgArray[pos + 2]));
		childPos = childPos >> 2;
		...
		while (true) {
			...
			if ((char) (this.dawgArray[childArrPos + 3]) == letter) {
				return (this.dawgArray[childArrPos + 2] & 1) > 0;
			}
	...
	public boolean isEndOFWord(final int currentNode) {
		return (this.dawgArray[(currentNode * 4) + 2] & 1) > 0;
	}

	/** Returns a hashmap with the child's letter as the key and the position its
	 * index as the value */
	public HashMap<Character, Integer> getChildren(final int currentNode) {
		final HashMap<Character, Integer> children = new HashMap<Character, Integer>();
		final int pos = currentNode * 4;
		int childPos = (this.unsignedToBytes(this.dawgArray[pos]) << 16) | (this.unsignedToBytes(this.dawgArray[pos + 1]) << 8) | (this.unsignedToBytes(this.dawgArray[pos + 2]));
		childPos = childPos >> 2;
		...
		while (true) {
			...
			children.put((char) (this.dawgArray[childArrPos + 3]), childPos);
